...
and `snakemake-config-var.yaml` (which is [preprocessed](https://github.com/RTXteam/RTX-KG2/issues/48#issuecomment-773508378) into `snakemake-config.yaml` and used by the Snakefile).


## `build-kg2-snakemake.sh`

`build-kg2-snakemake.sh` runs the commands necessary for `snakemake` to run without error, in addition to running the `snakemake` command itself and preprocessing `snakemake-config-var.yaml` to be used with the  `Snakefile`.
...
- **The `-F`** build flag **must** be left in the `snakemake` command when you are doing a full build as it ensures that every uncommented rule that connects to `Finish` (the first rule in the `Snakefile`) run, regardless of if the files generated by the rules already exist.
- **The `-R <rule_name>`** flag specifies a rule to be force run, and will run the rules required to generate any missing `input` files for the specified rules. The `output` files of the rule will be regenerated.
- **The `-n`** flag causes snakemake to do a dry-run, which will show the rules that will be run without actually running them. It also checks that the inputs and outputs of the Snakefile line up, but does NOT check the accuracy of `shell` commands. This is useful in conjuction with the other options, especially when learning how to use snakemake.

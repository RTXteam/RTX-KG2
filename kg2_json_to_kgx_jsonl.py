...
if os.path.exists('~/kg2-build/edges-kgx.jsonl'):
    os.remove('~/kg2-build/edges-kgx.jsonl')
if os.path.exists('~/kg2-build/kg2_json_to_kgx_tsv.log'):
    os.remove('~/kg2-build/kg2_json_to_kgx_tsv.log')
if os.path.exists('~/kg2-build/nodes-kgx.jsonl'):
    os.remove('~/kg2-build/nodes-kgx.jsonl')
...
output_edges_file = '~/kg2-build/edges_kgx.jsonl'
output_nodes_file = '~/kg2-build/nodes_kgx.jsonl'
edges = True
with jsonlines.open(input_file_name_full) as reader, jsonlines.open(output_edges_file, mode='w') as edges_writer, 
     jsonlines.open(output_nodes_file, 'w') as nodes_writer:
         for item in edges_fields: 
             edges_header += item + '\t'
         edges_writer.write(edge_header)
         for item in nodes_fields: 
             nodes_header += item + '\t'
         nodes_writer.write(nodes_header)
         print("--- Headers written ---", file=log_file)
         for item in reader:

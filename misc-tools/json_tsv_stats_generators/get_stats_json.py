"""
    Create a graph from a JSON input file (KG2) and calculate some stats
    for the graph. Write these stats to a JSON report file.

    Usage: get_stats_json.py <input_file> <output_file_location>
"""
...
def build_output_file_path(output_file_location):
    ...
    """
    file_name = "stats_report_json.json"
    output_file_path = ""
...
def initialize_results_dict(input_file_path, datetime_str):
    """
    Initialize the global 'results' dictionary.

    :param input_file_path: A string for the path to the input file
    ...
    results["input_files"] = {}
    results["input_files"]["paths"] = [input_file_path]
    results["created_graph"] = {}
    results["runtimes"] = {}


def build_graph(input_file_path):
    """
    :param input_file_path: A string path to the JSON input file
    :return graph: A graph built using the edges in the input file
    """
    global results

    # If current directory already has a pickled graph, read and return it
    if os.path.isfile("kg2.pickle"):
        graph = igraph.Graph.Read_Pickle(fname="kg2.pickle")
        return graph

    # If a pickled graph does not exist, build a graph using the input file
    with open(input_file_path) as file:
        # Convert JSON string to Python dictionary
        kg2_dict = json.loads(file.read())
        # Store the number of edges and number of nodes in the input file
        # to 'results'
        results["input_files"]["number_of_edges"] = len(kg2_dict["edges"])
        results["input_files"]["number_of_nodes"] = len(kg2_dict["nodes"])
        # Generate a tuple list specifying all edges in the input file
        edge_tuple_list = create_tuple_list(kg2_dict)
        ...
        graph.write_pickle(fname="kg2.pickle", version=-1)
        return graph


def create_tuple_list(kg2_dict):
    """
    :param kg2_dict: A dictionary representing the JSON input file
    :return edge_tuple_list: A list of tuples in which each tuple specifies
    the source vertex and target vertex of an edge
    """
    edge_tuple_list = []
    kg2_edges = kg2_dict["edges"]
    for edge in kg2_edges:
        source_vertex = edge["subject"]
        target_vertex = edge["object"]
        edge_tuple_list.append((source_vertex, target_vertex))
    return edge_tuple_list


def calculate_graph_stats(graph):
    """
    :param graph: The graph built from the JSON input file
    """
    global results
...
def plot_log_log_degree_distr(graph):
    ...
    plt.savefig("log_log_degree_distr_json.png", bbox_inches = "tight")
    plt.show()
    plt.draw()
...
def main():
    ...
    parser.add_argument("input_file", type=str,
                        help="path to the Knowledge Graph JSON input file")
    ...
    initialize_results_dict(input_file_path, datetime_str)

    # Build graph from the input file
    begin_time = datetime.datetime.now()
    graph = build_graph(input_file_path)

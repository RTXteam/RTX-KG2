...
## Highlights
 - Compared to the JSONLines build in [JSONLines_Time_Comparison.md](https://github.com/RTXteam/RTX-KG2/blob/master/docs/JSONLines_Time_Comparison.md), the build is notably shorter (by about 4 hours). A complete build, including KG2c (Babel-only) file generation, takes less than a day. This is very surprising given that no new optimizations have been made to the bottlenecks in each step. In particular, `Merge` got faster for no clear reason.
 - Despite this, SemMedDB now takes longer to extract. (Once again, despite no major code changes and the same version of the database). I suspect that the MySQL import function is a bottleneck.
 - Building KG2pre alongside KG2c creates disk space issues. We typically partition instances with 1028 GiB of storage. I had to delete the `binlog` files (~100GB that is irrelevant to us) generated by MySQL near the end of the build because it appeared that it would run out of disk space. (This requires manually using `sudo -i` to run `rm /var/lib/mysql/binlog*` at the right point.) Now that we have to download the Babel SQLite file and generate the KG2c files, we need more disk.
 - The build employs ample parallelism during the processing steps and thus uses more memory than previous versions of the build system. However, the `r5a.4xlarge` is still sufficient for build purposes.

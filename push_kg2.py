...
class push_kg2:
    ...
    def push_nodes(self, json_file):
        ...
        cypher_upload_nodes = 'WITH "' + json_file + '" AS file ' +\
    ...
    def push_edges(self, json_file, batch):
        """
        :param json_file: A string containing the path (or URL) to the json file
...
if __name__ == "__main__":
    ...
    parser.add_argument("-b", "--bolt", type=str, help="The neo4j bolt address", default = None)
    ...
    if node_flag:
        ...
        if args.debug:
            print(str(count), " nodes uploaded in ", str(time.time() - t0), " seconds.")
    if edge_flag:
        t0 = time.time()
